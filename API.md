...
|--------|----------|-------------|
| GET | `/healthz` | Unauthenticated health check for Docker and reverse proxies |

## Authentication

When `VIDCLAW_AUTH_USER` and `VIDCLAW_AUTH_PASSWORD` are set, all endpoints except `/healthz` require HTTP Basic Auth.
