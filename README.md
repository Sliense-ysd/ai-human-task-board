...
This fork includes a production Dockerfile for VPS deployment.
...
Set `VIDCLAW_AUTH_USER` and `VIDCLAW_AUTH_PASSWORD` before exposing the service outside localhost. When those variables are present, the dashboard and API require HTTP Basic Auth. `/healthz` remains unauthenticated for container and reverse-proxy health checks.
